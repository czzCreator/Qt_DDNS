...
struct Client
{
    int start(quint16 port, int count) {
        ...
        QUrl url;
        url.setScheme("http");
        url.setHost("192.168.1.6");
        url.setPath("/jeecg-boot/sys/serveLogin");
        url.setPort(port); 

        iurl   = url;
        icount = count;

        qDebug() << QString("czz ------------------  test URL:------") << url.toString();

        //itick.tick();    //no use tick example
        send();
    ...
    void send() {
        iclient.request(
                ...
                [this](QHttpRequest* req){
                    QJsonObject root{
                    {"username", "admin"},
                    {"password", "123456"}  // server computes sum of these values
                    };

                    qDebug() << QString("request  handler ------") << root;
...
public:
    ...
    QUrl         iurl;
    QHttpClient  iclient;
    //am::TickTock itick;       ////no use tick example
}; // struct client
///////////////////////////////////////////////////////////////////////////////
} // namespace client
///////////////////////////////////////////////////////////////////////////////
...
int
keep_alive_main(int argc, char ** argv) {
